    ...
    public class EmployeeController : Controller
    {
        ...
        public ActionResult AddOrEdit(int id = 0)
        {
            ...
            if (id != 0)
            {
                using (DBModel db = new DBModel())
                {
                    emp = db.Employee.Where(x => x.EmployedID == id).FirstOrDefault<Employee>();
                }
            }
            return View(emp);
        }
        ...
        public ActionResult Delete(int id)
        {
            try
            {
                using (DBModel db = new DBModel())
                {
                    Employee emp = db.Employee.Where(x => x.EmployedID == id).FirstOrDefault<Employee>();
